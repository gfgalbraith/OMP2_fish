...
# Many NA values - requires going into the csv files and checking what's going on. May even require looking at the videos. Some are genuine observations made outside of any particular transect but these shouldn't be included in the statistical analysis if they we not record on a transect.
...
unique(data$OpCode) # shows that some OpCodes have not been named correctly. Use 'filename' to apply SURVEY and maybe REEF. Edna and Juliette Cay require a bit of special attention using the OpCode variable. We also need to specify whether the survey was stereo or not. Refer to ROV transect full summary csv.

merged <- data %>% mutate(
  Site=str_sub(filename,9,-13),
  Site=substring(Site,12),
  Site = case_when(OpCode == "Edna_1_JUL2021" ~ "EdnaCay_1",
                   OpCode == "Edna_2_JUL2021" ~ "EdnaCay_2",
                   OpCode == "Edna_3_JUL2021" ~ "EdnaCay_3",
                   OpCode == "Juliette1_July2021" ~ "JulCay_1",
                   TRUE ~ Site),
  Video = case_when(str_detect(Site, c("Edna|Jul|HerSurp")) ~ "Single",
          TRUE ~ "Stereo"),        
  Transect=as.factor(Period),
  Length_mm=as.double(Length_mm),
  Survey=as.factor(case_when(grepl("FEB2021", filename) ~ "Feb_21",
                             grepl("FEB2022", filename) ~ "Feb_22",
                             grepl("JUL2021", filename)~ "Jul_21")),
  Site=as.factor(Site))

# Edna is three sites that have been analysed in one eventmeasure file - some manual edits done to the csv but still needs some attention here. Use OpCode which has been manual edited to reflect the three different sites.
...
merged <- merged %>% mutate ( 
  Reef = as.factor(case_when(grepl("Li", OpCode) ~ "Lihou",
                   ...
                   grepl("Edna", OpCode) ~ "Lihou",
                   grepl("Jul", OpCode) ~ "Lihou",
                   ...
                   grepl("HerS", OpCode) ~ "Herald's Surprise",
                   grepl("Mar", OpCode) ~ "Marion",)))

new.data<-select(merged,c(Survey,Reef,Site,Transect,Length_mm,Family,Genus,Species, Number,Comment, Video))
```
...
For fish this is easy as only fish observations will at least have "Family", "Genus" and/or "Species". Use this as a filter. NB making sure you at least ID a fish to family level is key.
...
trash <- trash %>% filter(
  str_detect(Comment, "ishing") | str_detect(Comment, "trash" | str_detect(Comment, "Fishing Line")))
...
coolstuff <-data %>% filter(
  is.na(Family) & is.na(Species) & !str_detect(Comment, c("cumber|fishing_line|trash|Fishing Line|Fishing line")))%>% 
  select(-Family, -Genus, -Species)

coolstuff

# Check that there's nothing in here that shouldn't be in the other subsets
