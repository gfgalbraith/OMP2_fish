...
```{r}
transect_data <-read_csv("../0_data/ROV_Transect_Summary_Analysis.csv")
...
transect_data <- transect_data %>% mutate(
  ...
  Survey=as.factor(case_when(month(Date) ==2 & year(Date) == 2021 ~ "Feb_21",
                             month(Date) ==2 & year(Date) == 2022 ~ "Feb_22",
                             month(Date) == 7 & year(Date) == 2021 ~ "Jul_21")),
  Site=as.factor(Site),
...
distinct(transect_data, T_ID) # 163 transects
distinct(fish, T_ID) #156 rows - some transects had no fish so this is fine for now
# Check fish observations that don't have a transect number - 0 all good
...
levels(fish$T_ID)
unique(fish$T_ID) #155 unique transect IDs in the fish
unique(transect_data$T_ID) #163 transects in the transect_data
...
# Paracentropyge multifasciata was named incorrectly
data3$Genus[2] <- "Paracentropyge"
data3$Species[2] <- "multifasciata"
data3$Family[2] <- "Pomacentridae"

data3
# everything else here just needs "sp" adding to the species column
# All NA species get "sp"
data3$Species[is.na(data3$Species)]<- "sp"
```

# wokring on subset data2
```{r}
...
# Missing pieces here are a random depth observation for H.marcosi and a funky trout.

# NOTE these length ests are in cm - converrt to mm to keep consistent with EM estimates! Will stuff uop your biomass later on.
...
data2$Length_mm[117] <- 200
data2$temp.species[117] <- "leopardus"
data2$temp.length[117] <- 200
...
funky_data$Genus[38] <- "Paracentropyge"
funky_data$Species[38] <- "multifasciata"
funky_data$Length_mm[38] <- 103
funky_data$Length_mm[34] <- 115


funky_data<-funky_data %>% mutate(
  Genus = case_when(Comment == "Neoniphon sammara" ~ "Neoniphon",
                    Comment == "Neoniphon opercularis" ~ "Neoniphon",
                    TRUE ~ Genus),
  Species = case_when(Comment == "Neoniphon sammara" ~ "sammara",
                      Comment == "Neoniphon opercularis" ~ "opercularis",
                      Comment == "ALBIPECTORALIS" ~ "albipectoralis",
  TRUE ~ Species))
...
```{r}
fin.data<-rbind(data,data2,data3,single.species,funky_data)
