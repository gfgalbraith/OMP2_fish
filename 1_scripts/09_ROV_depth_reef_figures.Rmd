...
anti_join(diversity,biomass, by = "T_ID")
...
fish <- fish %>% mutate ( 
  Region = as.factor(case_when(grepl("Lihou", Reef) ~ "Central Coral Sea",
                   ...
                   grepl("Herald", Reef) ~ "Central Coral Sea",
                    grepl("Herald's Surprise", Reef) ~ "Central Coral Sea",
...
P1<-ggplot(fish, aes(y=richness, x=Reef, fill =Depth_bin)) + 
  ...
  scale_fill_manual(values = depth.cols, name = "")+
  ylab(expression(Richness~("Species 150m"^-2)))+
  xlab("")+
...
ggsave(filename = "../3_figures/ROV_fish_REEF_richness.png", plot= P1,
       width =322, height=140 , units = "mm", dpi = 500, device = "png" )


ggsave(filename = "../3_figures/ROV_fish_REEF_richness.svg", plot= P1,
       width =322, height=140 , units = "mm", dpi = 500, device = "svg" )

#  %+-%SE is plus/minus standard error sign
```
...
P2<-ggplot(fish, aes(y=abundance, x=Reef, fill =Depth_bin)) + 
  geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=FALSE)+
  scale_fill_manual(values = depth.cols, name = "")+
  ylab(expression(Density~("n 150m"^-2)))+
  xlab("")+
...
ggsave(filename = "../3_figures/ROV_fish_REEF_abundance.svg", plot= P2,
       width =322, height=140 , units = "mm", dpi = 500, device = "svg" )
...
P3<-ggplot(fish, aes(y=shannon, x=Reef, fill =Depth_bin)) + 
  geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=FALSE)+
  scale_fill_manual(values = depth.cols, name = "")+
  ylab(expression(Diversity~("H' 150m"^2)))+
  xlab("")+
...
ggsave(filename = "../3_figures/ROV_fish_REEF_diversity.svg", plot= P3,
       width =322, height=140 , units = "mm", dpi = 500, device = "svg" )
...
P4<-ggplot(fish, aes(y=Total_biomass_KG, x=Reef, fill =Depth_bin)) + 
  geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=2, notch=FALSE)+
  scale_fill_manual(values = depth.cols, name = "")+
  ylab(expression(Biomass~("Kg 150m"^-2)))+
  xlab("")+
...
ggsave(filename = "../3_figures/ROV_REEF_biomass.svg", plot= P4,
       width =322, height=140 , units = "mm", dpi = 500, device = "pdf" )


```
 
# Look at individual biomass to identify outliers
```{r}
indi.fish<-read_csv("../2_outputs/ROV_fish_biomass.csv")

```
 
# Combined Figure - needs text size tweaking and only one REGION header
```{r}
ggarrange(P1,P2,P3,P4, ncol = 1, lables = c("A","B","C","D"))
